...
**Internal links:** When linking between documentation articles, use a path shaped like `sectionId/articleId` (for example, `mcp/mcp-concepts.md` or `api/running-tests-via-api.md`). Avoid file-relative links like `./...` or `../...`, since the docs UI routes by `sectionId` + `articleId`.
